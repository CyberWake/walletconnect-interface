...
class WCInterface {
  ...
  void clearSessions() {
    final v1Connections = _v1Client.connectedClients;
    final v2Connections = _v2Client.session.getAll();
    for (final element in v1Connections) {
      _v1Client.disconnect(element.client.sessionStore);
    }
    for (final element in v2Connections) {
      _v2Client.disconnect(topic: element.topic);
    }
  }
}
